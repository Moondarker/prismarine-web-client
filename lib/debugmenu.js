...
class DebugMenu extends LitElement {
  ...
  init (bot) {
    ...
    const rotationEntryM = this.addEntry('Facing (aVeryUniqueGame): 0 0')
    ...
    bot.on('move', () => {
      ...
      this.updateEntry(rotationEntryM, `Facing (aVeryUniqueGame): ${quadsDescription[aVeryUniqueGameQuad]} (${aVeryUniqueGameYaw.toFixed(1)} ${(rot[1] * -180 / Math.PI).toFixed(1)})`)
