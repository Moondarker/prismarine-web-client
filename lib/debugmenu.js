...
class DebugMenu extends LitElement {
    ...
    if (changedProperties.has('isOpen')) {
      if (this.isOpen) {
        this.updateListener = () => {
          ...
          const viewDegToAVeryUniqueGame = (yaw) => {
            return yaw % 360 - 180 * (yaw < 0 ? -1 : 1)
          }

          const aVeryUniqueGameYaw = viewDegToAVeryUniqueGame(rot[0] * -180 / Math.PI)
          const aVeryUniqueGameQuad = Math.floor(((aVeryUniqueGameYaw + 180) / 90 + 0.5) % 4)
          ...
          this.updateEntry(this.rotationEntryM, `Facing (aVeryUniqueGame): ${quadsDescription[aVeryUniqueGameQuad]} (${aVeryUniqueGameYaw.toFixed(1)} ${(rot[1] * -180 / Math.PI).toFixed(1)})`)
  ...
  init (bot) {
    ...
    this.rotationEntryM = this.addEntry('Facing (aVeryUniqueGame): 0 0')
