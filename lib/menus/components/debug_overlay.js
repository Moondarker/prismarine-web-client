...
class DebugOverlay extends LitElement {
  ...
  render () {
    ...
    const viewDegToAVeryUniqueGame = (yaw) => yaw % 360 - 180 * (yaw < 0 ? -1 : 1)
    ...
    return html`
      <div class="debug-left-side">
        ...
        <p>Facing (aVeryUniqueGame): ${quadsDescription[aVeryUniqueGameQuad]} (${aVeryUniqueGameYaw.toFixed(1)} ${(rot[1] * -180 / Math.PI).toFixed(1)})</p>
        <p>Light: ${skyL} (${skyL} sky)</p>
        <p>Biome: aVeryUniqueGame:${window.mcData.biomesArray[biomeId].name}</p>
